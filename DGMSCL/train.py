...
def main_work(args):
    best_acc1 = 0
    # tensorboard文件
    writer = SummaryWriter('D:/tensorboardFiles')

    if args.tag == 'date':
        local_date = time.strftime('%m.%d', time.localtime(time.time()))
        args.tag = local_date

    log_file = 'D:/TSADGNN/log/{}_gpu{}_{}_{}_exp.txt'.format(args.tag, args.gpu, args.arch, args.dataset)
    ...
    for epoch in range(args.epochs):
        ...
        # tensorboard可视化
        writer.add_scalar('10%SRS2-loss', loss_val_per, epoch)
        writer.add_scalar('10%SRS2-acc', acc_val_per, epoch)
    # 关闭tensorboard
    writer.close()
