...
def final_contrastive_loss(nb_trans, z, labels, alpha=0.34, beta=0.33, temporal_unit=0):
    loss = torch.tensor(0., device=z.device)
    labels = labels.to(device=z.device)
    d = 0
    while z.size(1) > 1:
        if alpha != 0:
            loss += alpha * weight_augmented_inter_graph_supervised_contrastive_loss(z, labels, nb_trans)
        if beta != 0:
            loss += beta * context_based_minority_class_aware_contrast_loss(z, labels, nb_trans)
        if d >=temporal_unit:
            if 1 - alpha - beta != 0:
...
def weight_augmented_inter_graph_supervised_contrastive_loss(z, nb_trans):
...
def context_based_minority_class_aware_contrast_loss(z, labels, nb_trans):
...
def supervised_contrastive_loss(z, labels, nb_trans):
    B, T = z.size(0)/nb_trans, z.size(1)
    if B == 1:
        return z.new_tensor(0.)
    labels_list = labels
    ...
    for key in count:
        ...
        logits_labels = torch.tril(logits_mask, diagonal=-1)[:, :-1]
        logits_labels += torch.triu(logits_mask, diagonal=1)[:, 1:]
        data_key = data_key.transpose(0, 1)  
        sim = torch.matmul(data_key, data_key.transpose(1, 2))  
        logits = torch.tril(sim, diagonal=-1)[:, :, :-1]  
        logits += torch.triu(sim, diagonal=1)[:, :, 1:]
        logits = -F.log_softmax(logits, dim=-1)
        logits = logits * logits_labels
